...
def add_comtrs_2005_2016_unfixed(year): #test to see if correcting the data changes it in some way 
    '''relatively simple since this dataset already includes the comtrs'''
    final_two_digits = str(year)
    final_two_digits = final_two_digits[-2:]
    # pdb.set_trace()
    # Extract data from counties 10, 15, 16, 54
    overall_data_fresno = pd.read_csv(str('pur_data_raw/pur' + str(year) +'/udc' + final_two_digits + '_10.txt'), sep = ',', error_bad_lines = False, warn_bad_lines = True)
    overall_data_kern = pd.read_csv(str('pur_data_raw/pur' + str(year) +'/udc' + final_two_digits + '_15.txt'), sep = ',', error_bad_lines = False, warn_bad_lines = True)
    overall_data_kings = pd.read_csv(str('pur_data_raw/pur' + str(year) +'/udc' + final_two_digits + '_16.txt'), sep = ',', error_bad_lines = False, warn_bad_lines = True)
    overall_data_tulare = pd.read_csv(str('pur_data_raw/pur' + str(year) +'/udc' + final_two_digits + '_54.txt'), sep = ',', error_bad_lines = False, warn_bad_lines = True)
    ...
    for dataset_num, dataset in tqdm(enumerate(full_dataset)):
        if dataset_num == 0:
            ...
            # pdb.set_trace()
            tlb_overall_data = pd.concat([tlb_overall_data, dataset])
    tlb_overall_data.to_csv(str('comtrs_pur_vals_year' + str(year) + 'unfixed.csv'), index = True )
...
def calculate_acres_pre_1990(year, crop_type, crop_iter, crop_list, tlb_overall_data): # for a given set of comtrs, calculate total tree crop acreage 
    ...
    for COMTRS_iter, COMTRS_value in enumerate(tqdm(COMTRS_list)): 
        ...
        try:
            for batch_num, individual_batch in enumerate(batch_IDs): #goes through the individual sites in the section 
                # pdb.set_trace()
...
def calculate_acres_1990_2016(year, crop_type, crop_iter, crop_list, tlb_overall_data): # for a given set of comtrs, calculate total tree crop acreage
    ...
    for COMTRS_iter, COMTRS_value in enumerate(tqdm(COMTRS_list)): 
        ...
        try:  # trys going through all IDs at this COMTRS- if none exists, will set total acreage of this crop to zero
            # print('THIS WORKS')
            # if (year == 2016) & (crop_type == 2006):
            #     pdb.set_trace()
            ...
            print('THIS is throwing and exception')
            # pdb.set_trace()
            total_in_COMTRS = 0 
            # pdb.set_trace()
    ...
    try:
        ...
        # if (year == 2016) & (crop_type == 2006):
        #     pdb.set_trace()
        #     print('final acreage number for crop 2006 is')
        #     print(sum(crop_acres_list.values))
        #     print('This line 455')
    ...
    if year > 1989:
        for crop_type in tqdm(crop_list):  # Runs for each crop type in calPIP database, then connects to larger calPIP array using COMTRS index 
            ...
            try:
                ...
                # if crop_type == '2006':
                    # pdb.set_trace()
                    # print('test our crop 2006 (oranges)')
            except:
                crop_iter = crop_iter + 1 
                print(f'crop2 dataframe may have been empty for this crop type number {crop_type}')    
...
def retrieve_data_for_irrigation_district(irrigation_district, normalized):
    ...
    for df_row, year in enumerate(tqdm(range(1974,2017))):    # editted here to include up to 2016 
        ...
        if year < 1990:
            ...
            # sum_grape = sum(crop_data_in_irrigation_district['29141'])  
            # sum_citrus = sum(crop_data_in_irrigation_district['2000'])  
            # sum_nuts = sum(crop_data_in_irrigation_district['3000'])  
            # sum_almond = sum(crop_data_in_irrigation_district['3001'])  
            # sum_walnut = sum(crop_data_in_irrigation_district['3009'])  
            # sum_apple = sum(crop_data_in_irrigation_district['4001'])  
            # sum_grape = sum(crop_data_in_irrigation_district['29141'])  
            # sum_grape = sum(crop_data_in_irrigation_district['29141'])  
            # sum_grape = sum(crop_data_in_irrigation_district['29141'])  
            # sum_grape = sum(crop_data_in_irrigation_district['29141'])  
            ...
            # sum_trees_some = sum_alfalfa + sum_nectarine + sum_grape + sum_citrus + sum_nuts + sum_almond + sum_walnut + sum_apple
            # # pdb.set_trace()
            # print('test stuff out here Oct 2')     
...
# print('done with compiling')
# pdb.set_trace()
# year = 2016
# compile_data_by_comtrs(year)
# pdb.set_trace()
# print('done with 2016 compiling ')
...
# year = 2016 
# # add_comtrs_2005_2016(year)
# add_comtrs_2005_2016_unfixed(year)  # check if this makes a difference Sep 28 
# pdb.set_trace()
